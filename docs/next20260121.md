# accuDaq 第二阶段开发规划

**文档版本**: 1.0
**创建日期**: 2026-01-21
**规划周期**: 8-12周

---

## 一、现状评估：MVP需求 vs 当前实现

### 1.1 整体完成度概览

| 模块 | MVP要求 | 当前状态 | 完成度 |
|------|---------|---------|--------|
| **基础架构** | VSCode扩展 | 独立Web应用 | 30% |
| **可视化编辑** | 无限画布+拖拽+连线 | ✅ 已实现 | 90% |
| **编译器** | .daq → Python | ✅ 已实现 | 95% |
| **组件库** | 11个核心组件 | 6个组件 | 55% |
| **设备管理** | Modbus+Mock+面板 | 仅Mock设备 | 30% |
| **UI设计器** | 拖拽式Dashboard | 固定布局 | 40% |
| **双模式** | 可视化↔代码 | 仅可视化 | 20% |
| **AI辅助** | Chat+代码生成 | ❌ 未实现 | 0% |

### 1.2 已完成功能详情

#### ✅ 可视化编辑器 (visual-editor/)
```
已实现：
├── App.tsx              # 主应用，节点/边状态管理
├── ComponentLibrary.tsx # 组件拖拽面板
├── DAQNode.tsx          # 自定义节点渲染
├── PropertyPanel.tsx    # 属性编辑面板
├── Toolbar.tsx          # 工具栏
└── Dashboard.tsx        # 运行时仪表盘
```

#### ✅ 编译器架构 (daq_core/compiler/)
```
已实现：
├── parser.py      # .daq JSON解析 → DAQProject
├── topology.py    # Kahn算法拓扑排序
├── codegen.py     # Python代码生成
└── compiler.py    # 编译器主流程
```

#### ✅ 组件系统 (daq_core/components/)
| 组件 | 类型 | 功能 | 状态 |
|------|------|------|------|
| MockDevice | Source | 模拟正弦/方波/随机数据 | ✅ |
| MQTTClient | Comm | MQTT订阅 | ✅ |
| MQTTPublisher | Comm | MQTT发布 | ✅ |
| MathOperation | Process | 数学运算(+-*/scale) | ✅ |
| Compare | Process | 比较逻辑(><==) | ✅ |
| CSVStorage | Sink | CSV数据存储 | ✅ |

#### ✅ Dashboard控件 (widgets/)
- LineChartWidget.tsx - 实时波形图
- GaugeWidget.tsx - 仪表盘

### 1.3 缺失功能分析

#### ❌ MVP要求但未实现

| 功能 | MVP描述 | 差距 | 优先级 |
|------|---------|------|--------|
| **VSCode集成** | 基于VSCode扩展 | 当前是独立Web应用 | 🔴高 |
| **Modbus TCP** | 工业协议读取 | 未实现 | 🔴高 |
| **设备管理面板** | 设备列表/状态指示 | 未实现 | 🔴高 |
| **拖拽Dashboard** | 所见即所得设计 | 当前固定布局 | 🔴高 |
| **LED指示灯** | 红/绿状态灯 | 未实现 | 🟡中 |
| **开关按钮** | 控制类控件 | 未实现 | 🟡中 |
| **数值输入框** | 控制类控件 | 未实现 | 🟡中 |
| **阈值报警组件** | 简单阈值判断 | 未实现 | 🟡中 |
| **Debug Print** | 调试打印组件 | 未实现 | 🟡中 |
| **全局变量** | UI变量绑定 | 未实现 | 🟡中 |
| **双模式切换** | 可视化→代码 | 未实现 | 🟡中 |
| **高亮执行** | 调试数据流 | 未实现 | 🟢低 |
| **AI Chat** | 代码解释/生成 | 未实现 | 🟢低 |

---

## 二、第二阶段开发目标

### 2.1 核心目标

**实现MVP验收标准**：用户能在10分钟内完成温度监控项目：
1. ✅ 新建"温度监控"项目
2. ✅ 添加虚拟温度传感器
3. ✅ 拖入逻辑节点（温度>30判断）
4. ✅ 设计Dashboard（波形图+报警灯）
5. ✅ 运行查看实时数据
6. ✅ 导出CSV数据

### 2.2 阶段划分

```
Phase 2.1 (Week 1-3)  → 补齐核心组件 + 设备管理
Phase 2.2 (Week 4-6)  → Dashboard设计器升级
Phase 2.3 (Week 7-9)  → VSCode集成 + 双模式
Phase 2.4 (Week 10-12) → AI辅助 + 调试功能
```

---

## 三、详细开发计划

### Phase 2.1：核心组件与设备管理（Week 1-3）🔴

#### Week 1：补齐Process/Sink组件

| 组件 | 工作量 | 实现要点 |
|------|--------|---------|
| ThresholdAlarm | 1天 | 输入value+threshold，输出alarm(boolean) |
| DebugPrint | 0.5天 | 输入any，打印到控制台 |
| GlobalVariable | 1天 | 读写全局变量，支持UI绑定 |
| DataBuffer | 1天 | 数据缓存，支持历史查询 |

**ThresholdAlarm 实现示例**：
```python
# daq_core/components/threshold_alarm.py
@ComponentRegistry.register
class ThresholdAlarmComponent(ComponentBase):
    component_name = "ThresholdAlarm"

    def _setup_ports(self):
        self.add_input_port("value", PortType.NUMBER)
        self.add_input_port("threshold", PortType.NUMBER)
        self.add_output_port("alarm", PortType.BOOLEAN)
        self.add_output_port("value_out", PortType.NUMBER)

    def process(self):
        value = self.get_input("value")
        threshold = self.config.get("threshold", 30)

        if value is not None:
            alarm = value > threshold
            self.set_output("alarm", alarm)
            self.set_output("value_out", value)
```

**前端组件定义**：
```typescript
// componentLibrary.ts 新增
{
    type: 'threshold_alarm',
    name: 'Threshold Alarm',
    category: 'logic',
    icon: '🚨',
    inputs: [
        { id: 'value', name: 'Value', type: 'number' }
    ],
    outputs: [
        { id: 'alarm', name: 'Alarm', type: 'boolean' },
        { id: 'value_out', name: 'Value', type: 'number' }
    ],
    defaultProperties: {
        threshold: 30,
        compare_type: 'greater'
    }
}
```

#### Week 2：Modbus TCP支持

| 任务 | 工作量 | 依赖 |
|------|--------|------|
| pymodbus集成 | 0.5天 | pip install pymodbus |
| ModbusClient组件 | 2天 | 读取保持寄存器 |
| 连接管理 | 1天 | 重连机制、错误处理 |
| 前端配置UI | 1天 | IP/端口/寄存器配置 |

**ModbusClient 组件**：
```python
# daq_core/components/modbus_client.py
from pymodbus.client import ModbusTcpClient

@ComponentRegistry.register
class ModbusClientComponent(ComponentBase):
    component_name = "ModbusClient"
    component_type = ComponentType.DEVICE

    def _setup_ports(self):
        self.add_output_port("value", PortType.NUMBER)
        self.add_output_port("connected", PortType.BOOLEAN)

    def start(self):
        super().start()
        self.client = ModbusTcpClient(
            self.config.get('host', '127.0.0.1'),
            port=self.config.get('port', 502)
        )
        self.client.connect()

    def process(self):
        if self.client.connected:
            register = self.config.get('register', 0)
            result = self.client.read_holding_registers(register, 1)
            if not result.isError():
                self.set_output("value", result.registers[0])
                self.set_output("connected", True)
```

#### Week 3：设备管理面板

**新增组件**：`DevicePanel.tsx`

```typescript
// visual-editor/src/components/DevicePanel.tsx
interface Device {
    id: string
    name: string
    type: 'mock' | 'modbus' | 'mqtt'
    status: 'connected' | 'disconnected' | 'connecting'
    config: Record<string, any>
}

const DevicePanel = () => {
    const [devices, setDevices] = useState<Device[]>([])

    return (
        <div className="device-panel">
            <h3>📡 设备管理</h3>
            <button onClick={addDevice}>+ 添加设备</button>

            {devices.map(device => (
                <DeviceCard
                    key={device.id}
                    device={device}
                    onConnect={() => connectDevice(device.id)}
                    onDisconnect={() => disconnectDevice(device.id)}
                />
            ))}

            {/* 实时数据监视窗口 */}
            <RawDataViewer selectedDevice={selectedDevice} />
        </div>
    )
}
```

**功能清单**：
- [x] 设备列表显示
- [x] 添加/删除设备
- [x] 连接/断开按钮
- [x] 状态指示灯 (🟢/🔴/🟡)
- [x] Raw Data Viewer（实时数据表格）
- [x] 设备配置编辑

---

### Phase 2.2：Dashboard设计器升级（Week 4-6）🔴

#### Week 4：Grid布局系统

**技术选型**：react-grid-layout

```bash
npm install react-grid-layout
```

**DashboardDesigner.tsx**：
```typescript
import GridLayout from 'react-grid-layout'

const DashboardDesigner = () => {
    const [layout, setLayout] = useState([
        { i: 'chart1', x: 0, y: 0, w: 6, h: 4 },
        { i: 'led1', x: 6, y: 0, w: 2, h: 2 }
    ])
    const [widgets, setWidgets] = useState<Widget[]>([])
    const [editMode, setEditMode] = useState(true)

    return (
        <div className="dashboard-designer">
            {/* 工具栏 */}
            <WidgetToolbar onAddWidget={addWidget} />

            {/* 可拖拽网格 */}
            <GridLayout
                layout={layout}
                cols={12}
                rowHeight={60}
                onLayoutChange={setLayout}
                isDraggable={editMode}
                isResizable={editMode}
            >
                {widgets.map(widget => (
                    <div key={widget.id}>
                        <WidgetRenderer widget={widget} />
                    </div>
                ))}
            </GridLayout>

            {/* 属性面板 */}
            <WidgetPropertyPanel
                widget={selectedWidget}
                variables={availableVariables}
            />
        </div>
    )
}
```

#### Week 5：新增Dashboard控件

| 控件 | 类型 | 功能 | 工作量 |
|------|------|------|--------|
| LEDWidget | 显示 | 红/绿指示灯 | 1天 |
| NumberDisplay | 显示 | 数字显示框 | 0.5天 |
| SwitchWidget | 控制 | 开关按钮 | 1天 |
| NumberInput | 控制 | 数值输入框 | 1天 |
| ProgressBar | 显示 | 进度条 | 0.5天 |

**LEDWidget 实现**：
```typescript
// widgets/LEDWidget.tsx
interface LEDWidgetProps {
    value: boolean
    label: string
    colorOn?: string   // 默认绿色
    colorOff?: string  // 默认红色
}

const LEDWidget = ({ value, label, colorOn = '#27ae60', colorOff = '#e74c3c' }: LEDWidgetProps) => {
    return (
        <div className="led-widget">
            <div
                className="led-indicator"
                style={{
                    width: 24,
                    height: 24,
                    borderRadius: '50%',
                    backgroundColor: value ? colorOn : colorOff,
                    boxShadow: value
                        ? `0 0 10px ${colorOn}`
                        : 'none'
                }}
            />
            <span className="led-label">{label}</span>
        </div>
    )
}
```

#### Week 6：变量绑定系统

**目标**：控件可以绑定"设备变量"或"全局变量"

```typescript
// 变量绑定数据结构
interface VariableBinding {
    type: 'device' | 'global' | 'node_output'
    path: string  // e.g., "mock_device.value" or "global.temperature"
}

// 变量选择器组件
const VariableSelector = ({ onSelect }: { onSelect: (binding: VariableBinding) => void }) => {
    const devices = useDevices()
    const globalVars = useGlobalVariables()
    const nodeOutputs = useNodeOutputs()

    return (
        <div className="variable-selector">
            <h4>选择数据源</h4>

            <div className="section">
                <h5>📡 设备变量</h5>
                {devices.map(d => (
                    <div key={d.id} onClick={() => onSelect({ type: 'device', path: `${d.id}.value` })}>
                        {d.name}.value
                    </div>
                ))}
            </div>

            <div className="section">
                <h5>🔗 节点输出</h5>
                {nodeOutputs.map(n => (
                    <div key={n.path} onClick={() => onSelect({ type: 'node_output', path: n.path })}>
                        {n.label}
                    </div>
                ))}
            </div>

            <div className="section">
                <h5>📦 全局变量</h5>
                {globalVars.map(v => (
                    <div key={v.name} onClick={() => onSelect({ type: 'global', path: v.name })}>
                        {v.name}
                    </div>
                ))}
            </div>
        </div>
    )
}
```

**Dashboard配置保存格式**：
```json
{
    "ui": {
        "widgets": [
            {
                "id": "widget-1",
                "type": "line_chart",
                "layout": { "x": 0, "y": 0, "w": 6, "h": 4 },
                "config": {
                    "title": "温度曲线",
                    "binding": {
                        "type": "node_output",
                        "path": "mock_device_1.value"
                    },
                    "color": "#4a90d9"
                }
            },
            {
                "id": "widget-2",
                "type": "led",
                "layout": { "x": 6, "y": 0, "w": 2, "h": 2 },
                "config": {
                    "label": "报警灯",
                    "binding": {
                        "type": "node_output",
                        "path": "threshold_alarm_1.alarm"
                    },
                    "colorOn": "#e74c3c",
                    "colorOff": "#27ae60"
                }
            }
        ]
    }
}
```

---

### Phase 2.3：VSCode集成与双模式（Week 7-9）🟡

#### Week 7-8：VSCode扩展核心

**目录结构**：
```
vscode-extension/
├── src/
│   ├── extension.ts        # 扩展入口
│   ├── webviewProvider.ts  # Webview面板管理
│   ├── commands.ts         # 命令注册
│   └── projectManager.ts   # 项目文件管理
├── webview/                # 嵌入的React应用
│   └── (visual-editor构建产物)
└── package.json
```

**核心命令注册**：
```typescript
// extension.ts
export function activate(context: vscode.ExtensionContext) {
    // 注册命令
    context.subscriptions.push(
        vscode.commands.registerCommand('accudaq.newProject', () => {
            createProject(context)
        }),
        vscode.commands.registerCommand('accudaq.openEditor', () => {
            openVisualEditor(context)
        }),
        vscode.commands.registerCommand('accudaq.compile', () => {
            compileProject()
        }),
        vscode.commands.registerCommand('accudaq.run', () => {
            runProject()
        }),
        vscode.commands.registerCommand('accudaq.switchMode', () => {
            toggleEditorMode()
        })
    )

    // 注册Webview Provider
    context.subscriptions.push(
        vscode.window.registerWebviewViewProvider(
            'accudaq.visualEditor',
            new DAQEditorProvider(context.extensionUri)
        )
    )
}
```

**Webview通信**：
```typescript
// webviewProvider.ts
class DAQEditorProvider implements vscode.WebviewViewProvider {
    resolveWebviewView(webviewView: vscode.WebviewView) {
        webviewView.webview.options = {
            enableScripts: true
        }

        // 加载React应用
        webviewView.webview.html = this.getHtmlContent()

        // 监听来自Webview的消息
        webviewView.webview.onDidReceiveMessage(message => {
            switch (message.type) {
                case 'save':
                    this.saveProject(message.data)
                    break
                case 'compile':
                    this.compileProject()
                    break
                case 'run':
                    this.runEngine()
                    break
            }
        })
    }
}
```

#### Week 9：双模式切换

**模式切换逻辑**：
```typescript
type EditorMode = 'visual' | 'code'

const ModeSwitch = () => {
    const [mode, setMode] = useState<EditorMode>('visual')

    const switchToCode = async () => {
        // 1. 编译当前可视化项目生成代码
        const code = await compileToCode()

        // 2. 在VSCode中打开生成的Python文件
        vscode.commands.executeCommand('vscode.open',
            vscode.Uri.file(generatedCodePath)
        )

        setMode('code')
    }

    return (
        <div className="mode-switch">
            <button
                className={mode === 'visual' ? 'active' : ''}
                onClick={() => setMode('visual')}
            >
                📊 可视化模式
            </button>
            <button
                className={mode === 'code' ? 'active' : ''}
                onClick={switchToCode}
            >
                💻 代码模式
            </button>
        </div>
    )
}
```

**MVP限制说明**：
- ✅ 支持：可视化 → 代码（单向转换）
- ❌ 不支持：代码 → 可视化（反向转换留待后续版本）

---

### Phase 2.4：AI辅助与调试（Week 10-12）🟢

#### Week 10-11：AI Chat集成

**侧边栏Chat窗口**：
```typescript
// components/AIChat.tsx
const AIChat = () => {
    const [messages, setMessages] = useState<Message[]>([])
    const [input, setInput] = useState('')

    const sendMessage = async () => {
        // 添加用户消息
        const userMsg = { role: 'user', content: input }
        setMessages([...messages, userMsg])

        // 调用LLM API
        const response = await fetch('/api/ai/chat', {
            method: 'POST',
            body: JSON.stringify({
                message: input,
                context: {
                    components: availableComponents,
                    project: currentProject
                }
            })
        })

        const result = await response.json()
        setMessages([...messages, userMsg, { role: 'assistant', content: result.reply }])
        setInput('')
    }

    return (
        <div className="ai-chat">
            <div className="messages">
                {messages.map((msg, i) => (
                    <div key={i} className={`message ${msg.role}`}>
                        {msg.content}
                    </div>
                ))}
            </div>
            <div className="input-area">
                <input
                    value={input}
                    onChange={e => setInput(e.target.value)}
                    placeholder="问我任何问题..."
                />
                <button onClick={sendMessage}>发送</button>
            </div>
        </div>
    )
}
```

**后端AI服务**：
```python
# api/ai_service.py
from openai import OpenAI  # 或使用 Anthropic

SYSTEM_PROMPT = """
你是accuDaq数据采集系统的AI助手。

可用组件：
- MockDevice: 模拟数据源（正弦/方波/随机）
- ModbusClient: Modbus TCP设备读取
- MathOperation: 数学运算
- ThresholdAlarm: 阈值报警
- CSVStorage: CSV数据存储

你可以：
1. 解释代码含义
2. 生成简单的Python脚本
3. 推荐适合的组件

请提供简洁、可执行的代码示例。
"""

def chat(message: str, context: dict) -> str:
    client = OpenAI()
    response = client.chat.completions.create(
        model="gpt-4",
        messages=[
            {"role": "system", "content": SYSTEM_PROMPT},
            {"role": "user", "content": message}
        ]
    )
    return response.choices[0].message.content
```

#### Week 12：调试功能

**高亮执行模式**：
```typescript
// 调试状态管理
interface DebugState {
    isDebugging: boolean
    currentNodeId: string | null
    portValues: Record<string, Record<string, any>>
    speed: 'slow' | 'normal' | 'fast'
}

const DebugOverlay = ({ nodeId, portValues }: { nodeId: string, portValues: Record<string, any> }) => {
    return (
        <div className="debug-overlay">
            {Object.entries(portValues).map(([port, value]) => (
                <div key={port} className="port-value">
                    <span className="port-name">{port}:</span>
                    <span className="port-data">{JSON.stringify(value)}</span>
                </div>
            ))}
        </div>
    )
}

// 节点高亮样式
const nodeStyle = (isExecuting: boolean) => ({
    border: isExecuting ? '3px solid #f39c12' : '1px solid #333',
    boxShadow: isExecuting ? '0 0 15px rgba(243, 156, 18, 0.5)' : 'none',
    animation: isExecuting ? 'pulse 1s infinite' : 'none'
})
```

---

## 四、里程碑与验收标准

### 4.1 里程碑计划

| 里程碑 | 时间 | 交付物 | 验收标准 |
|--------|------|--------|---------|
| **M1** | Week 3 | 核心组件+设备管理 | 可连接Modbus设备并显示数据 |
| **M2** | Week 6 | Dashboard设计器 | 可拖拽设计自定义仪表盘 |
| **M3** | Week 9 | VSCode集成 | 可在VSCode中完整操作项目 |
| **M4** | Week 12 | MVP完整版 | 通过10分钟验收测试 |

### 4.2 MVP验收测试流程

```
测试场景：温度监控项目（10分钟内完成）

步骤1：新建项目 [< 1分钟]
├── 打开VSCode
├── 执行 "DAQ: New Project" 命令
└── 选择"温度监控"模板

步骤2：添加虚拟传感器 [< 1分钟]
├── 打开设备管理面板
├── 点击"添加设备"
├── 选择"Mock Device"
└── 配置波形类型为"正弦波"

步骤3：配置逻辑节点 [< 3分钟]
├── 拖入Mock Device节点
├── 拖入Threshold Alarm节点
├── 连接 value → input
└── 设置阈值为30

步骤4：设计Dashboard [< 3分钟]
├── 切换到Dashboard设计器
├── 拖入LineChart控件
├── 拖入LED控件
├── 绑定数据源

步骤5：运行验证 [< 2分钟]
├── 点击"运行"按钮
├── 观察波形图实时更新
├── 验证温度>30时LED变红
└── 停止运行，检查CSV文件
```

---

## 五、技术栈与依赖

### 5.1 新增依赖

**Python后端**：
```bash
pip install pymodbus>=3.0.0   # Modbus支持
pip install flask>=2.0.0      # API服务
pip install openai>=1.0.0     # AI集成（可选）
```

**前端**：
```bash
npm install react-grid-layout  # Dashboard布局
npm install zustand           # 状态管理
```

**VSCode扩展**：
```bash
npm install @vscode/webview-ui-toolkit
```

### 5.2 项目结构更新

```
accuDaq/
├── visual-editor/
│   └── src/
│       ├── components/
│       │   ├── DevicePanel.tsx      # 新增
│       │   ├── DashboardDesigner.tsx # 新增
│       │   ├── AIChat.tsx           # 新增
│       │   └── widgets/
│       │       ├── LEDWidget.tsx    # 新增
│       │       ├── SwitchWidget.tsx # 新增
│       │       └── NumberInput.tsx  # 新增
│       └── stores/
│           └── projectStore.ts      # 新增（Zustand）
│
├── daq_core/
│   └── components/
│       ├── modbus_client.py         # 新增
│       ├── threshold_alarm.py       # 新增
│       ├── debug_print.py           # 新增
│       └── global_variable.py       # 新增
│
├── vscode-extension/
│   └── src/
│       ├── extension.ts             # 更新
│       ├── webviewProvider.ts       # 新增
│       └── commands.ts              # 新增
│
└── api/
    ├── server.py                    # 新增
    └── ai_service.py                # 新增
```

---

## 六、风险与应对

| 风险 | 影响 | 概率 | 应对措施 |
|------|------|------|---------|
| Modbus设备兼容性 | 高 | 中 | 先支持标准协议，提供调试工具 |
| VSCode Webview性能 | 中 | 中 | 优化渲染，考虑虚拟化 |
| AI API稳定性 | 低 | 中 | 降级为可选功能 |
| 进度延期 | 中 | 中 | AI辅助功能可延后 |

**优先级调整策略**：
- 🔴 高优先级（必须完成）：组件库、设备管理、Dashboard设计器
- 🟡 中优先级（尽量完成）：VSCode集成、双模式
- 🟢 低优先级（可延后）：AI辅助、高亮调试

---

## 七、第二阶段完成后状态

### 7.1 组件库对照

| MVP要求 | 第一阶段 | 第二阶段 |
|---------|---------|---------|
| 模拟信号发生器 | ✅ MockDevice | ✅ |
| Modbus读取 | ❌ | ✅ ModbusClient |
| 数学运算 | ✅ MathOperation | ✅ |
| 比较逻辑 | ✅ Compare | ✅ |
| 阈值报警 | ❌ | ✅ ThresholdAlarm |
| Debug Print | ❌ | ✅ DebugPrint |
| 全局变量 | ❌ | ✅ GlobalVariable |
| CSV存储 | ✅ CSVStorage | ✅ |

### 7.2 功能对照

| MVP要求 | 完成状态 |
|---------|---------|
| VSCode扩展形态 | ✅ |
| .daq项目文件 | ✅ |
| 双模式切换 | ✅（单向） |
| 可视化画布编辑 | ✅ |
| 设备管理面板 | ✅ |
| 拖拽式Dashboard | ✅ |
| 变量绑定 | ✅ |
| 本地运行 | ✅ |
| CSV存储 | ✅ |
| AI辅助 | ✅（基础版） |

---

## 八、后续版本展望（V3.0）

第二阶段完成后，后续可考虑：

1. **双向转换**：代码 → 可视化图
2. **更多协议**：OPC UA, CANopen
3. **高级算法**：FFT, 滤波器, PID
4. **云平台**：远程监控、数据上云
5. **移动端**：React Native Dashboard

---

## 附录：快速参考

### A. 编译器映射表更新

```python
# codegen.py - NODE_TYPE_MAPPING 新增
NODE_TYPE_MAPPING = {
    # 现有
    "daq:mock_device": "MockDevice",
    "daq:mqtt_subscribe": "MQTTSubscriber",
    "daq:mqtt_publish": "MQTTPublisher",
    "daq:math": "MathOperation",
    "daq:compare": "Compare",
    "daq:csv_storage": "CSVStorage",

    # 第二阶段新增
    "daq:modbus_tcp": "ModbusClient",
    "daq:threshold_alarm": "ThresholdAlarm",
    "daq:debug_print": "DebugPrint",
    "daq:global_variable": "GlobalVariable",
}
```

### B. 每周交付检查表

**Week 1-3**:
- [ ] ThresholdAlarm组件实现并测试
- [ ] DebugPrint组件实现并测试
- [ ] GlobalVariable组件实现并测试
- [ ] ModbusClient组件实现并测试
- [ ] DevicePanel.tsx 完成
- [ ] 设备管理API接口

**Week 4-6**:
- [ ] react-grid-layout集成
- [ ] LEDWidget实现
- [ ] SwitchWidget实现
- [ ] NumberInput实现
- [ ] 变量绑定系统
- [ ] Dashboard配置保存/加载

**Week 7-9**:
- [ ] VSCode扩展Webview集成
- [ ] 命令注册（new/open/compile/run）
- [ ] 双模式切换UI
- [ ] 代码生成优化

**Week 10-12**:
- [ ] AI Chat侧边栏
- [ ] LLM API集成
- [ ] 高亮执行模式
- [ ] MVP验收测试通过

---

**文档结束**

*accuDaq开发团队 | 2026-01-21*
